...
export NUMBER_OF_NODES="9"		# Used to setup multiple nodes
...
docker-machine create --driver amazonec2 --amazonec2-access-key $AWS_ACCESS_KEY_ID \
	...
	--amazonec2-region $AWS_REGION --amazonec2-instance-type t2.micro \
        $NODE_ROOT_SIZE \
	--engine-opt dns=8.8.8.8 aws-swarm-keystore
eval "$(docker-machine env aws-swarm-keystore)"
...
docker-machine create --driver amazonec2 --amazonec2-access-key $AWS_ACCESS_KEY_ID \
	...
	--swarm --swarm-master --swarm-strategy "spread" \
        $NODE_ROOT_SIZE \
	--swarm-discovery="consul://$(docker-machine ip aws-swarm-keystore):$AWS_SWARM_PORT" \
	--engine-opt="cluster-store=consul://$(docker-machine ip aws-swarm-keystore):$AWS_SWARM_PORT" \
...
do
	...
	docker-machine create --driver amazonec2 --amazonec2-access-key $AWS_ACCESS_KEY_ID \
		...
		--swarm --swarm-discovery="consul://$(docker-machine ip aws-swarm-keystore):$AWS_SWARM_PORT" \
		--engine-opt="cluster-store=consul://$(docker-machine ip aws-swarm-keystore):$AWS_SWARM_PORT" \
